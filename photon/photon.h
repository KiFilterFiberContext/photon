...
namespace photon::core
{
	struct softbp
	{
		...
		static int hook_with_entry( const char* lib, const char* sym, void* callback )
		{
			uintptr_t address = ( uintptr_t ) get_lib_symbol( lib, sym );
			if ( address == 0 )
				return -1;
		...
		static int midhook_with_entry( const char* lib, const char* sym, mid_hook_info_t minfo )
		{
			uintptr_t address = ( uintptr_t ) get_lib_symbol( lib, sym ) + minfo.instr_offset;
			if ( address == 0 )
				return -1;
